...
class TownControllerTest {
	...
	private List<TownResponse> towns = new ArrayList<>();
	...
	public void initToken() {
		...
		for (String name : names) {
			towns.add(getTownResponse(name, "대한민국"));
		}
	}
	...
	void findTownsByUser() throws Exception {
		// given
		Long userId = 1L;
		given(townService.findTownsByUser(userId)).willReturn(towns);
		...
		then(townService).should().findTownsByUser(userId);
	}
